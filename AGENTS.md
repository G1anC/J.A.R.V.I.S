<!-- Generated by `mycelium install agents` from ~/.mycelium/rules and ~/.mycelium/skills. Edits here are overwritten on the next run; change the source instead. -->

Run `mycelium recap` at the start of the session to see recent work and active claims.
